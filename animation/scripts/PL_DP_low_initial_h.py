...
args = [newt_make_step, GV['dx'], 3, GV['Q'], 0.1, None, False, GV['N']]
h_initial = np.ones(GV['N']) * 0.01
h_initial[0] = GV['h0']
t_span = GV['t-span']
sol = solve_ivp(fun=FVM_RHS, args=(args,), y0=h_initial, t_span=t_span, method='BDF', rtol=1e-6, atol=1e-8, events=[steady_state])
...
newt_animation.save_animation('../gifs/newt_lower_initial_h_DP015')