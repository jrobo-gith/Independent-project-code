...
def magnitude_of_deviation(steady_state:np.ndarray, startup_flow:np.ndarray) -> float:
    ...
    error_array = np.abs(startup_flow - steady_state)
    error_average = np.mean(error_array)
    return error_average ## TEMPORARY