...
if __name__ == '__main__':
    A = 0.0
    args = [make_step, GV['dx'], 3, GV['Q'], A, None, False, GV['N']]
    h_initial = np.ones(GV['N']) * GV['h0']
    t_span = GV['t-span']

    unstable.terminal = True
    steady_state.terminal = True
    vectorized_FVM_RHS = vmap(FVM_RHS)
    sol = solve_ivp(fun=vectorized_FVM_RHS, args=(args,), y0=h_initial, t_span=t_span, method='BDF', rtol=1e-6, atol=1e-8,
                    events=[unstable, steady_state], vectorized=True)
